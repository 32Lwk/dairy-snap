...
4. **AgentMemory**: Remove keys invalidated by edits; upsert **short** domain key/values still clearly stated in the transcript — not long-term prose. Domains allowed include `orchestrator`, `school`, `calendar_daily`, `calendar_work`, `calendar_social`, `hobby`, `romance`. If the transcript implies the user **denied** a previously learned calendar-intent rule, delete or weaken the corresponding AgentMemory rows and adjust long-term bullets.
