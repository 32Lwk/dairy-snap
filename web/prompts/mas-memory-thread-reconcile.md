...
2. **Short-term**: Keep only bullets still supported by the transcript for **this entry date**. Delete obsolete rows by `id`. When (re)writing bullets, stay **brief**; **do not** prefix every line with a date. For **relative / vague** calendar phrases only, add **`(YYYY-MM-DD)`** (Tokyo) immediately after the phrase, with **`HH:mm`** only if the transcript gives a clock time.
