...
import {
  APP_ENTRIES_NAV_SIDEBAR_LG_PT_BELOW_FIXED_HEADER_COMPACT,
  APP_FIXED_HEADER_SUBTRACT_FROM_100DVH_COMPACT,
  APP_FIXED_HEADER_TOP_FOR_OVERLAY_COMPACT,
...
export function EntriesNavLayoutShell({
  ...
  /** `/entries/[date]` は固定ツールバーあり。モバイルドロワーをその下から出して被りを防ぐ */
  const drawerBelowFixedHeader = /^\/entries\/[^/]+/.test(pathname);
    ...
    <EntriesNavDrawerContext.Provider value={ctx}>
      {open ? (
        <button
          type="button"
          className={[
            "fixed left-0 right-0 bottom-0 z-[55] bg-zinc-950/45 lg:hidden",
            drawerBelowFixedHeader ? APP_FIXED_HEADER_TOP_FOR_OVERLAY_COMPACT : "top-0",
          ].join(" ")}
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden lg:flex-row lg:items-stretch">
        <aside
          ...
          className={[
            ...
            "fixed left-0 z-[60] border-r shadow-xl transition-transform duration-300 ease-out",
            drawerBelowFixedHeader
              ? `${APP_FIXED_HEADER_TOP_FOR_OVERLAY_COMPACT} ${APP_FIXED_HEADER_SUBTRACT_FROM_100DVH_COMPACT}`
              : "top-0 h-[100dvh] max-h-[100dvh] pt-[env(safe-area-inset-top,0px)]",
            "pb-[env(safe-area-inset-bottom,0px)] lg:static lg:top-auto lg:z-auto lg:h-auto lg:min-h-0 lg:max-h-full lg:max-w-none lg:self-stretch lg:border-r lg:pb-0 lg:shadow-none",
            drawerBelowFixedHeader
              ? APP_ENTRIES_NAV_SIDEBAR_LG_PT_BELOW_FIXED_HEADER_COMPACT
              : "lg:pt-0",
        ...
        >
          <div className="shrink-0 border-b border-zinc-200 px-4 py-2 sm:px-6 sm:py-2.5 lg:px-3 lg:py-3 dark:border-zinc-800">
            ...
            <h2 className="mt-0.5 text-sm font-semibold leading-tight text-zinc-900 sm:mt-1 dark:text-zinc-50">
              エントリ一覧
            </h2>
