...
export async function POST(req: NextRequest) {
  ...
  const agentMemory = await loadAgentMemory(session.user.id, "calendar_daily");
  const result = await runCalendarDailyAgent({ ...parsed.data, agentMemory, userId: session.user.id });
