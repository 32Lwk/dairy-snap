...
export async function POST(req: NextRequest) {
  ...
  const agentMemory = await loadAgentMemory(session.user.id, "calendar_social");
  const result = await runCalendarSocialAgent({ ...parsed.data, agentMemory, userId: session.user.id });
