...
export async function POST(req: NextRequest) {
  ...
  if (parsed.data.persona.avoidTopics.includes("romance")) {
    return NextResponse.json({ error: "romance topic is blocked by avoidTopics" }, { status: 403 });
  }

  const agentMemory = await loadAgentMemory(session.user.id, "romance");
  const result = await runRomanceAgent({ ...parsed.data, agentMemory, userId: session.user.id });
